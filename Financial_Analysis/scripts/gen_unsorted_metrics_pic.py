...
# Plot 'Percent_Growth' in the default order (ascending index)
...
# Plot each of the other metrics using the index for the x-axis
metrics = ['Profit_Margin', 'Asset_Turnover', 'Financial_Leverage', 'ROA', 'ROE']
for i, metric in enumerate(metrics):
    ax = axs[(i + 1) // 3, (i + 1) % 3]  # Determine subplot position
    ax.plot(data['Percent_Growth'].values, data[metric].values, marker='o', linestyle='-', color='r')
    ax.set_title(metric)
    ax.set_xlabel('Index')
...
plt.savefig('pics/metric_outlier_reduction.png')
